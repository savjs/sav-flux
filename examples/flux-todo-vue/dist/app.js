    ...
    for (; i < length; i++) {
        ...
        if (options != null) {
            for (name in options) {
                ...
                copy = options[name];
                if (target !== copy) {
                    if (deep && copy && (isObject$1(copy) || (copyIsArray = isArray(copy)))) {
                        if (copyIsArray) {
                            copyIsArray = false;
...
function clone (val) {
    if (isArray(val)) {
        return extend$1(true, [], val)
    }
    else if (isObject$1(val)) {
        return extend$1(true, {}, val)
    }
    return extend$1(true, [], [val])[0]
}
...
function bindEvent(target) {
	...
	prop(target, 'listen', function (event, fn) {
		target.on(event, fn);
		return function () {
		    target.off(event, fn);
		};
	...
	prop(target, 'emit',  function (event) {
		...
		if (_events[event]) {
		    var list = _events[event].slice(), fn;
		    while ((fn = list.shift())) {
		        fn.apply(void 0, args);
		    }
		}
...
function Flux (opts) {
  ...
  val('proxys', {});
  val('opts', opts);
  initUse$1(flux)([initUtil, bindEvent, initPromise, initCloneThen,
    initState$1, initCommit, initDispatch, initProxy$1,
    initDeclare]);
}

function initProp (flux) {
  return {
    get: function get (key, val, opts) {
      if ( opts === void 0 ) opts = {};

      opts.get = val;
      Object.defineProperty(flux, key, opts);
    },
    val: function val (key, val, opts) {
      if ( opts === void 0 ) opts = {};
...
function initUtil (ref) {
  ...
  prop$$1.val('extend', extend$1);
}

function initState$1 (ref) {
  var prop$$1 = ref.prop;
  var emit = ref.emit;
  ...
  prop$$1.val('getState', function () { return clone$$1(state); });

  prop$$1.val('replaceState', function (newState) {
    state = newState;
    return cloneThen(newState).then(function (cloneState) {
      emit('replace', cloneState);
      return cloneState
    })
  });

  prop$$1.val('updateState', function (changedState, slice) {
...
function initProxy$1 (ref) {
  var prop$$1 = ref.prop;
  var proxys = ref.proxys;

  prop$$1.val('proxy', function (name, value) {
...
function initCloneThen (ref) {
  ...
  if (!probe.MessageChannel) {
    prop$$1.val('cloneThen', function (value) {
...
var Todo = {
render: function(){with(this){return _h('div',{staticClass:"todolist"},[_h('h4',["TODO LIST",_h('i',["(create times "+_s(count)+")"])])," ",_h('ul',[_l((todoList),function(child){return _h('li',[_h('label',[_h('input',{attrs:{"type":"checkbox"},domProps:{"checked":child.isCompleted},on:{"change":function($event){toggleCompleted(child)}}})," ",_h('span',[_s(child.title)])])," ",_h('button',{on:{"click":function($event){removeItemById(child.id)}}},["x"])])})])," ",_h('input',{directives:[{name:"model",rawName:"v-model",value:(newText),expression:"newText"}],attrs:{"type":"text"},domProps:{"value":_s(newText)},on:{"input":function($event){if($event.target.composing)return;newText=$event.target.value}}})," ",_h('button',{on:{"click":function($event){createNew(newText)}}},["Add"])])}},
staticRenderFns: [],
		computed: mapGetters([
			'todoList'
		]),
...
vue_runtime_common$1.use(FluxVue);
...
var app = new vue_runtime_common$1(Object.assign({}, {vaf: new FluxVue({
		flux: flux,
		mixinActions: true
	}),
	el: '#app'},
